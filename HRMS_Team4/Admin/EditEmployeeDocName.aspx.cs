...
{
    public partial class EditEmployeeDocName : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    Response.Redirect("FetchEmployeeDocName.aspx");
                }
            }
        }
        ...
        private void LoadDocumentData(string id)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("usp_addEmployeeDocNames_GetById", con))
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            if (Request.QueryString["id"] != null)
            {
                using (SqlConnection con = new SqlConnection(connString))
                {
                    using (SqlCommand cmd = new SqlCommand("usp_addEmployeeDocNames_Update", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
